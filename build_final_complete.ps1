...
$pyinstallerArgs = @(
    ...
    # Cryptography - FIXED: Added missing modules
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path "dist\ClinicalLabSoftware\ClinicalLabSoftware.exe") {
        ...
        # Additional Windows 7 compatibility check
        Write-Host "`nWindows 7 Compatibility Notes:" -ForegroundColor Yellow
        Write-Host "- Built as 32-bit application (win32)" -ForegroundColor White
        Write-Host "- Make sure Windows 7 has Service Pack 1 installed" -ForegroundColor White
        Write-Host "- Install Visual C++ Redistributable on target machine" -ForegroundColor White
        Write-Host "- Test on Windows 7 before distribution" -ForegroundColor White
    }
} else {
